...
class AbstableManager:
    '''Manager class for handling different abstable files at runtime.'''
    
    _instance = None
    _abstables = {
        'corsika': 'abstable.npz',
        'm5': 'abstable_MODTRAN_bottomup.npz',
        'm5_new': 'abstable_MODTRAN_new.npz'
    }
    
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(AbstableManager, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance
    
    def __init__(self):
        if not self._initialized:
            self._current_abstable = 'corsika'
            self._loaded_data = {}
            self._initialized = True
    
    def set_abstable(self, name: str):
        '''Set the current abstable to use.'''
        if name not in self._abstables:
            available = list(self._abstables.keys())
            raise ValueError(f"Unknown abstable '{name}'. Available options: {available}")
        self._current_abstable = name
    
    def get_current_data(self):
        '''Get the data for the currently selected abstable.'''
        if self._current_abstable not in self._loaded_data:
            filename = self._abstables[self._current_abstable]
            with as_file(files('CHASM.data') / filename) as file:
                self._loaded_data[self._current_abstable] = np.load(file)
        return self._loaded_data[self._current_abstable]
    
    @property
    def ecoeff(self):
        return self.get_current_data()['ecoeff']
    
    @property
    def l_list(self):
        return self.get_current_data()['wavelength']
    
    @property
    def altitude_list(self):
        return self.get_current_data()['height']


# Global abstable manager instance
_abstable_manager = AbstableManager()


def useAbstable(name: str):
    '''Set the abstable to use for atmospheric attenuation calculations.
    
    Parameters:
    name: str - The name of the abstable to use. Available options:
        'corsika' - uses abstable.npz
        'm5' - uses abstable_MODTRAN_bottomup.npz
        'm5_new' - uses abstable_MODTRAN_new.npz
    '''
    _abstable_manager.set_abstable(name)
...
class Attenuation(ABC):
    ...
    @property
    def ecoeff(self):
        return _abstable_manager.ecoeff
    
    @property
    def l_list(self):
        return _abstable_manager.l_list
    
    @property
    def altitude_list(self):
        return _abstable_manager.altitude_list
    ...
    #     # calculate the number density for the atmosphere at each step
    #     N = self.atm.number_density(self.axis.h) / 1.e6 #convert to particles/cm^3
    #     # calculate the step size in cm
    #     dh = self.axis.dh * 1.e2 #convert to cm
    #     for i, y in enumerate(self.yield_array):
    #         # calculate the Rayleigh scattering cross section at each step
    #         cs = self.rayleigh_cs(self.axis.h, y.l_mid)
    #         # calculate the log fraction surviving each step
    ...
    def vertical_log_fraction(self) -> np.ndarray:
        ...
        log_fraction_array = np.empty_like(self.yield_array, dtype='O')
        for i, y in enumerate(self.yield_array):
            # select tau for nearest wavelength in l list
            ecoeffs = self.ecoeff[np.abs(y.l_mid-self.l_list).argmin()]
            # interpolate tau onto the simulation altitude grid to find tau(h)
            e_of_h = np.interp(self.axis.h, self.altitude_list, ecoeffs)
            # determine the fraction surviving from ground to altitude h
            frac_surviving = np.exp(-e_of_h)
            # determine the fraction surviving each step by differencing
            frac_step_surviving = 1. - np.diff(frac_surviving[::-1], append = 1.)[::-1]
            # take the natural log of the fraction surviving each step
