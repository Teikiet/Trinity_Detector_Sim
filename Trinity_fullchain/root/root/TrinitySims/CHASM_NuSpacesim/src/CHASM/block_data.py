...
from .eventio_types import EventioType, Float, UShort, Double, String, Int, Varint, Varstring, Short, PhotonsData, ThreeByte
...
def make_event_header(sig: ShowerSignal) -> EventHeaderData:
    ...
    return EventHeaderData(
        total_energy = Float(sig.energy),
        zenith = Float(sig.axis.zenith),
        azimuth = Float(sig.axis.azimuth),
        obs_levels = [Float(sig.counters.vectors[:,2].min()*100)] * 10
    )
...
def make_longitudinal(sig: ShowerSignal) -> LongitudinalData:
    ...
    return LongitudinalData(nthick=UShort(N_thick), nch=N_ch, ng=n_g)
