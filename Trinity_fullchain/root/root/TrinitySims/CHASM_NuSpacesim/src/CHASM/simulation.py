...
#        self.transformToGrISU()    

    def set_shower_energy(self, eSh: float) -> None:
         self.energy = eSh

    def transformToGrISU(self) -> None:
        cz = np.sqrt(1.0 - self.cx**2 - self.cy**2)
#        print('Here')
        i_y = self.y
        self.y = -1.0*self.x
        self.x = -1.0*i_y
    ...
    def rand_xy(self) -> tuple[np.ndarray]:
        ...
        x = a * np.cos(incoming_angle) * np.cos(phi) + r * np.sin(incoming_angle) * np.sin(phi) - self.counters.input_radius * 100 * sinQ/cosQ * np.cos(incoming_angle)
        y = a * np.sin(incoming_angle) * np.cos(phi) - r * np.cos(incoming_angle) * np.sin(phi) - self.counters.input_radius * 100 * sinQ/cosQ * np.sin(incoming_angle)
        return x, y
    ...
    def set_shower_energy(self, energy: float) -> None:
      self.energy = energy
