...
class GRootWriter
{
 private:
   ...
   TTree *fTree;
   std::string treeBaseName;

   char version[20];
   unsigned int fEventNumber;
   unsigned int fPrimaryType;
   float        fPrimaryEnergy;
   float        fEmergenceProbability;
   float        fXcore;
   float        fYcore;
   float        fXcos;
   float        fYcos;
 ...
 public:
   ...
   GRootWriter( TFile *tfile, const unsigned int &iTelID, 
		const std::string &treeBaseName, const bool &storePhotonDcos = false, 
   ...
   int addEvent(const unsigned int &eventNumber, const unsigned int &primaryType, 
		const double &primaryEnergy,const double &emergenceProbability,
    const ROOT::Math::XYZVector &vSCore, const ROOT::Math::XYZVector &vSDCore, const double &xSource,
