...
void GArrayTel::setPrimary(const ROOT::Math::XYZVector &vSCorec,
                           ...
                           const double &energyc, const double &EmProbabilityc,
                           const double &WobbleTNc,
  ...
  if (debug) {
    ...
    *oLog << "        primary emergence probability  "  << fEmProbability << endl;
  ...
  fSrcRelToCameraX = -fSrcRelToTelescopeX;
  fSrcRelToCameraY = fSrcRelToTelescopeY;
  ...
  if (debug) {
    ...
    *oLog << "        srcX/Y wrt camera " 
          << fSrcRelToCameraX*(TMath::RadToDeg()) 
          << "  " << fSrcRelToCameraY*(TMath::RadToDeg()) << endl;
    ...
    *oLog << "        Core Location grd.coor  " << endl;
    GUtilityFuncts::printGenVector(vSCoreGC); *oLog << endl;
...
bool GArrayTel::setPhoton(const ROOT::Math::XYZVector &pGrd,
  ...
  iPhotType      = pType;
  iPhotTelHitNum = pTel;
  //if photon is first to be ray traced from a bunch, sets this boolean to draw it if ray drawing is enabled
  bnchFirstPh = phBnchNum == 0;
  ...
  const bool relLocFinite = std::isfinite(vPhotonRelLocTC.X()) &&
                            std::isfinite(vPhotonRelLocTC.Y()) &&
                            std::isfinite(vPhotonRelLocTC.Z());
  const bool dcosFinite = std::isfinite(vPhotonDcosTC.X()) &&
                          std::isfinite(vPhotonDcosTC.Y()) &&
                          std::isfinite(vPhotonDcosTC.Z());

  if (!relLocFinite || !dcosFinite) {
    // Skip problematic photons to avoid propagating NaNs through the optics trace.
    if (debug || debugShort) {
      *oLog << "         Skipping photon with invalid telescope coordinates (telID "
            << telID << ")" << endl;
      *oLog << "             relLocTC: ";
      GUtilityFuncts::printGenVector(vPhotonRelLocTC); *oLog << endl;
      *oLog << "             dcosTC  : ";
    ...
    bOnCamera = false;
    vPhotonCameraLoc = ROOT::Math::XYZVector();
    vPhotonCameraDcos = ROOT::Math::XYZVector();
    fTotalPhotonTime = 0.0;
    return false;
  }
...
void GArrayTel::makeTelescopeTest(const string& testfile) {
  ...
  int numDegBins;
  if (debug) *oLog << "   telType  " << telType << endl;
  ...
  if (debug) *oLog << "    estimated ideal transit time, normal incidence  "
	<< idealTransitTime  << endl;
  ...
  if (debug) {
     *oLog <<      " vX.size() " << vX.size() << endl;
     for (int j=0;j<nPhotons;j++) {
       *oLog <<      "  vX  vY  vZ  " << vX.at(j) << " " 
	     << vY.at(j) << " " << vZ.at(j) << endl;
    }
  }
  ...
  for (unsigned ideg=0;ideg<1;ideg++){//vDeg.size();ideg++) {
	...
	*oLog << "         photonLoc  ";       
      ...
      *oLog << "         photonDir  ";       
      ...
      onCamera = tel->getCameraPhotonLocation(&cameraLoc,&cameraDir,&cameraTime,true);
      cout << onCamera <<endl;
      if (onCamera) {
