...
bool GReadPhotonGrISU::setInputFile(const string &infile) {
  ...
  if (sInFileStr=="") {
    ...
    // Force stream reset to ensure proper state
    pInStream->clear();  // Clear any error flags
    pInStream->seekg(0, ios::beg);  // Seek to beginning of file
    
    // Verify stream is in good state
    if (!pInStream->good()) {
      cerr << "    -- GReadPhotonGrISU::setInputFile " << endl;
      cerr << " stream not in good state after reset, attempting to reopen " << sInFileStr << endl;
      delete pInStream;
      pInStream = new ifstream(sInFileStr.c_str());
      pInStream->clear();
      pInStream->seekg(0, ios::beg);
    }
  }
...
bool GReadPhotonGrISU::getPrimary(ROOT::Math::XYZVector *pCore, 
				  ...
				  double *Zn, double *energy, double *EmProbability,
  ...
  *particleType = iParticleType; // from header
  ...
  if (eRecType == SREC) {
    ...
    is >> c >> fSEnergy >> fSXcore >> fSYcore >> fSXcos >> fSYcos
       >> tmp >> iSSeed[0] >> iSSeed[1] >> iSSeed[2]>> fSEmProbability;
...
bool GReadPhotonGrISU::getPhoton(ROOT::Math::XYZVector *pGrd,
  ...
  if (eRecType == PREC) {
    ...
    is >> c >> xGrd >> yGrd >> xcos >> ycos >> fPHgtEmiss
       >> fPTime >> fPWaveLgt >> iPType >> iPTel >> nPhtsBnch;  
    ...
    //vPGrd.SetXYZ(xGrd,yGrd,fPHgtEmiss);
