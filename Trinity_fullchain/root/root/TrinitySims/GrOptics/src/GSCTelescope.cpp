...
void GSCTelescope::buildTelescope()
{
  ...
  *oLog << " CameraFlag " << bCameraFlag << endl;
  ...
  if (gPrimRefl!=0) {
    primaryMirror->SetReflectance(std::shared_ptr<TGraph> (gPrimRefl));
  }
  ...
  */
  ...
  if (gSeconRefl!=0) {
    secondaryMirror->SetReflectance(std::shared_ptr<TGraph> (gSeconRefl));
  }
  ...
  */
  ...
  if (kCameraFlag ) {
   ...
   if (kMAPMTRefIndex) {
      ARefractiveIndex *targ = new ARefractiveIndex(kMAPMTRefIndex,0);
      mapmtInputWindow->SetRefractiveIndex(std::shared_ptr<ARefractiveIndex> (targ));
    }
    else {
      AGlassCatalog *tcat = new AGlassCatalog("/home/jordan/simbox/ROBAST/ROBAST-3.2.0/misc/schottzemax-20180601.agf");
      std::shared_ptr<ARefractiveIndex> bk7 = tcat->GetRefractiveIndex("N-BK7");
