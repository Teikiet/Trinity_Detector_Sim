...
void GSegSCTelescope::addSecondaryMirror(const char*name, SegmentedMirror *mirror) {
  ...
  TGraph * graph = makeReflectivityGraph(iReflect);
  mir->SetReflectance(std::shared_ptr<TGraph> (graph));
...
void GSegSCTelescope::addEntranceWindow() {
  ...
  ARefractiveIndex *targ = new ARefractiveIndex(fEntranceWindowN,0);
  ewindLen->SetRefractiveIndex(std::shared_ptr<ARefractiveIndex> (targ));
