...
bool GSimulateOptics::startSimulations(const int &numShowers,
                                         const int &numPhotons, const int &numPhotonsBnch) {
  ...
  int numPhBnchTmp = iNPhotonsBnch;
  if(iNPhotonsBnch < 0) numPhBnchTmp = 1;
  ...
  for (int i=0; i<numShTmp; ++i) {
    ...
    primaryFlag = reader->getPrimary(&vSCore,&vSDcosGd,
                                     &fAzPrim,&fZnPrim,
				     &fPrimaryEnergy,&fEmProbability,&fPrimaryType,
    ...
    for (iterArrayTel=mArrayTel->begin();
      ...
      iterArrayTel->second->setPrimary(vSCore,vSDcosGd,fAzPrim,fZnPrim,
                                       fEnergy,fEmProbability,fWobbleTN,fWobbleTE,fLatitude);
      if (debug) {
	*oLog << " after set Primary " << fWobbleTN*(TMath::RadToDeg()) << " "
    ...
    for (int j = 0;j<numPhTmp;++j) {
      ...
      photonFlag = reader->getPhoton(&vPhotonGrdLoc,&vPhotonDCosGd,
                                     ...
                                     &iPhotTelHitNum, &fPhotBnch);

      if (!photonFlag) {
        if (j < 5) {
          *oLog << "      PhotonIdx " << j << " flag=false (no more photons)" << endl;
        }
      }
      ...
      if(iNPhotonsBnch > fPhotBnch) numPhBnchTmp = fPhotBnch; 
      if (numPhBnchTmp < 1) {
        numPhBnchTmp = 1;
      }
      ...
      // skip if telescope not included in the array
        ...
        *oLog << "      PhotonIdx " << j
              << " tel=" << iPhotTelHitNum
              << " bunch=" << fPhotBnch
              << " numPhBnchTmp=" << numPhBnchTmp
              << " telFound=" << (iterAT != mArrayTel->end())
              << endl;
       }

       if (iterAT != mArrayTel->end() ) {
        for(int pBN = 0; pBN<numPhBnchTmp; pBN++){
          ...
          if (!photonInjected) {
            if ((j < 5) && (pBN == 0)) {
              *oLog << "        Skipping photon for tel " << iPhotTelHitNum
                    << " due to invalid direction/location" << endl;
            }
            continue;
          }

          if ((j < 5) && (pBN == 0)) {
            *oLog << "        Counting photon for tel " << iPhotTelHitNum << endl;
          }
    ...
    *oLog << "    EventNumber " << fEventNumber << "   nPhotons "
      << nPhotons << endl;
    *oLog << "    LinesRead   " << reader->getLinesRead() << endl;
