...
struct Cline {
  ...
  vector<double> vTelPosition; //!< Position of telescope
  vector<double> vTelRotation; //!< Telescope rotation
};
...
/*!  Function to reconcile Command line (Cline) and Tel Factories.
 */
int updateTelescopeFactoryDetails(vector<TelFactory *> *vTelFac,
  map<int,TelDetails *> *mTelDetails,
  const Cline &cline);
...
int main(int argc, char *argv[]) {
  ...
  *oLog << endl 
  ...
  updateTelescopeFactoryDetails(&vTelFac,&mTelDetails,cline);
  //////////////////////////// output file here
  // TEST SKIP
  ...
  for (unsigned i = 0;i< vTelFac.size(); ++i) {
    ...
    else if ( vTelFac.at(i)->telType == TDEM) {
      readerTDEM = new GReadTDEMStd(vTelFac.at(i)->configFile);
      TDEMFac = new GTDEMTelescopeFactory(*readerTDEM,vTelFac.at(i)->editFile);
    }
  }
  ...
  for (mIter=mTelDetails.begin();mIter!=mTelDetails.end();mIter++) {
	...
	*oLog << "READY TO DRAW TELESCOPE AND QUIT" << endl;
    ...
    else if (telType==TDEM) {

      GTelescope *tel = TDEMFac->makeTelescope(telId,telStd);
      tel->setPrintMode(*oLog,printMode);
      if (bDrawRayFlag) {
        tel->setRayPlotMode(eRayType);
      }
  ...
  *oLog << "================= starting simulations " << endl << endl;
  ...
  siO->startSimulations(pilot.nShower,pilot.nPhoton,pilot.nPhotonBnch);
...
int readCommandLine(int argc, char *argv[], Cline *cline) {
  ...
  for (int i = 1;i<argc;i++) {
    *oLog << "COMMANDLINE " << clArg.at(i) << endl;
    ...
    else if(clArg.at(i)=="-pos"){
      for(int j = 0; j<4;j++){
        double pos = atof(clArg.at(i+1).c_str());
        cline->vTelPosition.push_back(pos);
        i++;
      }
    }
    else if(clArg.at(i)=="-rot"){
      for(int j = 0; j<3;j++){
        double rot = atof(clArg.at(i+1).c_str());
		    cline->vTelRotation.push_back(rot);
	      i++;
      }
    }
...
int commandLineHelp() {
  ...
  *oLog << "    -pos <TelID> <X> <Y> <Z> in m"<< endl;
  *oLog << "    -rot <TelID> <Azimuth> <Zenith> in deg"<< endl;
...
int pilotPrint(const Pilot &pilot) {
  ...
  *oLog << "         nShower / nPhoton  " << pilot.nShower
       << " / " << pilot.nPhoton<< " / "<<pilot.nPhotonBnch<<endl;
...
int readPilot(Pilot *pilot) {
  ...
  while (pi->get_line_vector(tokens) >=0) {
    ...
    if (tokens.size() == 2) {
      ...
      pilot->nPhoton = atoi(tokens.at(1).c_str());
      pilot->nPhotonBnch = atoi(tokens.at(2).c_str());
    }
  }  
  flag = "LOGFILE";
  pi->set_flag(flag);
...
/****************************** end of updatePilot ******************************/
int updateTelescopeFactoryDetails(vector<TelFactory *> *vTelFac,
  map<int,TelDetails *> *mTelDetails,const Cline &cline){
  if(cline.vTelPosition.size()>0){
    int telID = (int)cline.vTelPosition[0];
    double xl,yl,zl;
    xl = cline.vTelPosition[1];
    yl = cline.vTelPosition[2];
    zl = cline.vTelPosition[3];
    (*mTelDetails)[telID]->telLocGrd.SetCoordinates(xl,yl,zl);
  }
  if(cline.vTelRotation.size()>0){
    int telID = (int)cline.vTelRotation[0];
    double xoffset, yoffset;

    xoffset = cline.vTelRotation[1];
    yoffset = cline.vTelRotation[2];
    (*mTelDetails)[telID]->bFixedPtFlag = true;
    (*mTelDetails)[telID]->az_fixPt = xoffset * (TMath::DegToRad());
    (*mTelDetails)[telID]->zn_fixPt = yoffset * (TMath::DegToRad());
  }
