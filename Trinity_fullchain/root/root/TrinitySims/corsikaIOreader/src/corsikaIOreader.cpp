...
int main( int argc, char** argv )
{
    ...
    while( i++ < argc )
    {
        ...
        {
            // cout << "setting random seed to " << iTemp2 << endl;
            fSeed = atoi( iTemp2.c_str() );
            // cout << "random seed for Cherenkov photon wavelengths now set to: " << fSeed << endl;
            i++;
        }
    ...
    if( !bstdout )
    {
        // edit ERS 11/17 -- print seed used for Cherenkov photon wavelengths directly. GetSeed function returns a array size which is not useful here.
        cout << "SEED (for Cherenkov photon wavelengths): " << fSeed << endl;
    }
    if( !bstdout )
    {
        ...
        switch( block_header.type )
        {
            ...
            case IO_TYPE_MC_EVTH:
                ...
                if( !have_atm_profile )
                {
                    ...
                    if( iatmo <= 0 )
                    {
                        iatmo = 61;    /* US standard atmosphere */
                    }
                    try
                    {
                ...
                array.shower_sim.emergence_probability = evth[272];
                ...
                array.shower_sim.XTelTC = evth[269];
                array.shower_sim.YTelTC = evth[270];
                array.shower_sim.ZTelTC = evth[271];
...
//                        {
                            ...
                            Chphoton.photons = bunches[ibunch].photons;
