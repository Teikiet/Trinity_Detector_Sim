 ...
 *  @date    1997 to 2010
 *  @date    @verbatim CVS $Date: 2012/12/03 08:23:08 $ @endverbatim
 *  @version @verbatim CVS $Revision: 1.2 $ @endverbatim
 */

/* ================================================================ */

#include "../inc/initial.h"      /* This file includes others as required. */
#include "../inc/io_basic.h"     /* This file includes others as required. */
#include "../inc/mc_tel.h"
...
int print_tel_block (IO_BUFFER *iobuf)
{
   ...
   memcpy(txt,&idata,4);
   txt[4] = '\0';
   for (i=1; i<len && i<sizeof(data)/sizeof(data[0]); i++)
      data[i] = get_real(iobuf);

   if ( strncmp(txt,"RUNH",4) == 0 )
      printf("Corsika run header\n");
   else if ( strncmp(txt,"RUNE",4) == 0 )
      printf("Corsika run end\n");
   else if ( strncmp(txt,"EVTH",4) == 0 )
   {
      double E = data[3];
      int ptype = Nint(data[2]);
      printf("Corsika event header: primary of type %d and energy %f GeV.\n",
         ptype, E);
   }
   else if ( strncmp(txt,"EVTE",4) == 0 )
      printf("Corsika event end\n");
   else
      printf("Unknown CORSIKA block of type %s.\n", txt);
...
int write_photo_electrons (IO_BUFFER *iobuf, int array, int tel, int npe, 
   int pixels, int *pe_counts, int *tstart, double *t)
{
   IO_ITEM_HEADER item_header;
   int i, nonempty;
...
int read_photo_electrons (IO_BUFFER *iobuf, int max_pixels, int max_pe,
   int *array, int *tel, int *npe, int *pixels,
   int *pe_counts, int *tstart, double *t)
{
   IO_ITEM_HEADER item_header;
...
int print_photo_electrons (IO_BUFFER *iobuf)
{
   ...
   printf("Photo-electrons for telescope no. %d in array %d with "
          "%d p.e. in %d pixels of which %d are non-empty.\n",
